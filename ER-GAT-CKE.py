...
parser.add_argument("--embedding_dim_GMF", type=int, default=64, help="dimension of embedding in GMF submodel")
...
args.lambda_ind_fairness = 10
args.lambda_reward=1.0

cudnn.benchmark = True

def load_pretrained_data(dataset, pre_model):
    pretrain_path = './trained/%s/%s.npz' % (dataset,pre_model)
...
class NeuMF(nn.Module):
    def __init__(self, user_num, item_num, embedding_dim_GMF, embedding_dim_MLP, hidden_layer_MLP,
        ...
        dataset = 'amazon-cellphone'
        if args.model_type=='kgat':
            pre_model = 'kgat_si_sum_bi_l3'
        elif args.model_type=='cke':
            pre_model = 'cke'
        pretrain_data = load_pretrained_data(dataset, pre_model)

        user_embeds=pretrain_data['user_embed']
        if args.model_type == 'kgat':
            product_embeds=pretrain_data['item_embed'][0:item_num,:]
        elif args.model_type == 'cke':
            product_embeds = pretrain_data['item_embed']
        # print(pretrain_data)
        # print(pretrain_data['user_embed'])
        # print(pretrain_data['item_embed'])
...
if __name__ == "__main__":
    ...
    method = model_type+'101pid' +'-'+str(args.alpha)+'-' +str(args.beta)+'-'+str(args.batch_size)+'-'+str(args.lambda_ind_fairness)
    ...
    #train_loader = torch.from_numpy(np.array(train_loader))
    #test_loader=torch.from_numpy(np.array(test_loader))
    ...
    for epoch in range(args.epochs):
        ...
        for user, item, label in test_loader:
            ...
            for j in range(sample_size):
                ...
                # print(ranking)
                #log_model_prob[i][j] = IndividualFairnessLoss.compute_log_model_probability(args, scores, ranking)
